...
class Model(nn.Module):
    ...
    def forward(self, input_x, input_r, e2triple, triple2e, r2triple, is_training=False, input_y=None):
        ...
        E = self.E
        x_ori_i = input_x
        x_ori_i = torch.nn.functional.one_hot(x_ori_i.long(), self.E).unsqueeze(1)
        x_ori_i = x_ori_i.repeat(1, self.L, 1)
        ...
        for t in range(self.T):
            ...
            x = states[-1]  # [b, L, E]
            s_i, s_h, s_t = vectorized_operation(x, (e2triple[0], triple2e[1], r2triple[0], e2triple[2], w), E, self.r_size,
                                                            topk_pruning=self.c, is_max=True, 
                                                            weight=self.weight, use_topk=self.use_topk)  # [b, L, E]
            s, _ = torch.max(torch.stack([s_i, s_h, s_t], dim=0), dim=0)
