...
def create_graph(kg, device):
    ...
    triple2id = {}
    for i in range(i_h.shape[0]):
        h = i_h[i].item()
        r = i_r[i].item()
        t = i_t[i].item()
        triple = (h, r, t)
        triple2id[triple] = i
    return e2triple, triple2e, r2triple, triple2id


def mask_data(values, batches, triple2id, r_size, score):
    for item in batches:
        h = item['h']
        r = item['r']
        t = item['t']
...
def calculate_hits_full(state, batches, e2triple, triple2e, r2triple, graph, graph_inv, dataset, hits, use_eql=False):
    ...
    if len(r_mask_x) != 0:
        ...
        input_x_oh = torch.nn.functional.one_hot(input_x[:, 1], dataset.entity_dict.shape[0])
    ...
    if len(r_mask_y) != 0:
        ...
        input_y_oh = torch.nn.functional.one_hot(input_y[:, 1], dataset.entity_dict.shape[0])
...
def valid_process(valid_data, dataset, model, e2triple, triple2e, r2triple, triple2id, graph_entity,
    ...
    with torch.inference_mode():
        ...
        for i in tqdm(range(batch_num)):
            ...
            mask_data(e2triple[2], batches, triple2id, dataset.relation_size, False)
            state = model(input_x, input_r, e2triple, triple2e, r2triple, is_training=False)
            mask_data(e2triple[2], batches, triple2id, dataset.relation_size, True)
...
def test(dataset, graph_entity, e2triple, triple2e, r2triple, triple2id, option):
    model = Model(len(dataset.relation2id), option.step, option.length, dataset.entity_dict.shape[0],
                    ...
                    option.use_gpu, option.dropout, option.top_k, option.top_k_mask, option.use_soft)
    ...
    states = torch.load(model_save_path, map_location=torch.device('cpu'))
    states_new = {}
    for key in states:
        states_new[key.replace('module.', '')] = states[key]
    model.load_state_dict(states_new)
    if option.use_gpu:
        model = model.cuda()
        torch.cuda.empty_cache()
    if option.use_soft: model.weight.cpu()
    mrr, hit_1, hit_3, hit_10, _ = valid_process(dataset.test_data, dataset, model, e2triple, triple2e,
                                                     r2triple, triple2id, graph_entity, option, name='Test')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--top_k_mask', default=1000, type=int)
    ...
    parser.add_argument('--min_time', default=5, type=float)
    ...
    e2triple, triple2e, r2triple, triple2id = create_graph(dataset.kg, device)
    graph_entity = None
