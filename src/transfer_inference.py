...
def tf2torch_lstm(tf_kernel, tf_bias, input_size, hidden_size):
    """
    Convert TF BasicLSTMCell (kernel, bias) to PyTorch LSTM format.
    Args:
        tf_kernel: np.ndarray, shape [input_size+hidden_size, 4*hidden_size]
        tf_bias:   np.ndarray, shape [4*hidden_size]
    Returns:
        W_ih, W_hh, bias_ih, bias_hh (torch tensors)
    """

    W_ih = tf_kernel[:input_size, :].T    # [4*hidden_size, input_size]
    W_hh = tf_kernel[input_size:, :].T    # [4*hidden_size, hidden_size]

    # ---- gate reorder ----
    def reorder_gates(w):
        # TF BasicLSTMCell: [i, g, f, o]
        # PyTorch:          [i, f, g, o]
        i, g, f, o = np.split(w, 4, axis=0)
        return np.concatenate([i, f, g, o], axis=0)

    W_ih = reorder_gates(W_ih)
    W_hh = reorder_gates(W_hh)
    bias = reorder_gates(tf_bias)

    W_ih = torch.from_numpy(W_ih)
    W_hh = torch.from_numpy(W_hh)
    bias_ih = torch.from_numpy(bias)
    bias_hh = torch.zeros_like(bias_ih)

    return W_ih, W_hh, bias_ih, bias_hh
...
def test(dataset, graph_entity, e2triple, triple2e, r2triple, option):
    ...
    from_model_ckpt = option.trained_path

    
    if option.model_name == 'NeuralLP':
        tf_weights = np.load('{}/ckpt/model.weights.npy'.format(from_model_ckpt), allow_pickle=True).item()
        input_size = 128
        hidden_size = 128
        embeddings = torch.from_numpy(tf_weights['Variable:0'])
        W = torch.from_numpy(tf_weights['Variable_1:0'].T)
        b = torch.from_numpy(tf_weights['Variable_2:0']).T.squeeze(-1)
        model_weights_new = {'emb.weight': embeddings, 'linear.weight': W, 'linear.bias': b}
        kernel_fw = tf_weights['rnn/multi_rnn_cell/cell_0/lstm_cell/kernel:0']
        bias_fw = tf_weights['rnn/multi_rnn_cell/cell_0/lstm_cell/bias:0']
        W_ih_fw, W_hh_fw, bias_ih_fw, bias_hh_fw = tf2torch_lstm(
            kernel_fw,
            bias_fw,
            input_size=input_size,
            hidden_size=hidden_size
        )
        model_weights_new['lstm.weight_ih_l0'] = W_ih_fw
        model_weights_new['lstm.weight_hh_l0'] = W_hh_fw
        model_weights_new['lstm.bias_ih_l0'] = bias_ih_fw
        bias_hh_fw[input_size: 2 * input_size] = torch.ones_like(bias_hh_fw[input_size: 2 * input_size])
        model_weights_new['lstm.bias_hh_l0'] = bias_hh_fw
        ...
        W = torch.from_numpy(tf_weights['Variable_1:0'].T)
        b = torch.from_numpy(tf_weights['Variable_2:0']).T.squeeze(-1)
        w = torch.from_numpy(tf_weights['weights'].squeeze(-1))
        w = torch.permute(w, [3, 0, 1, 2])
        model_weights_new = {'emb': embeddings, 'linear.weight': W, 'linear.bias': b, 'w': w}
        for l in range(option.length):  
            kernel_fw = tf_weights['f_{}/fw/multi_rnn_cell/cell_0/lstm_cell/kernel:0'.format(l)]
            bias_fw = tf_weights['f_{}/fw/multi_rnn_cell/cell_0/lstm_cell/bias:0'.format(l)]
            W_ih_fw, W_hh_fw, bias_ih_fw, bias_hh_fw = tf2torch_lstm(
                kernel_fw,
                bias_fw,
                input_size=input_size,
            ...
            model_weights_new['lstm.{}.weight_ih_l0'.format(l)] = W_ih_fw
            model_weights_new['lstm.{}.weight_hh_l0'.format(l)] = W_hh_fw
            model_weights_new['lstm.{}.bias_ih_l0'.format(l)] = bias_ih_fw
            bias_hh_fw[input_size: 2 * input_size] = torch.ones_like(bias_hh_fw[input_size: 2 * input_size])
            model_weights_new['lstm.{}.bias_hh_l0'.format(l)] = bias_hh_fw

            kernel_bw = tf_weights['f_{}/bw/multi_rnn_cell/cell_0/lstm_cell/kernel:0'.format(l)]
            bias_bw = tf_weights['f_{}/bw/multi_rnn_cell/cell_0/lstm_cell/bias:0'.format(l)]
            W_ih_bw, W_hh_bw, bias_ih_bw, bias_hh_bw = tf2torch_lstm(
                kernel_bw,
                bias_bw,
                input_size=input_size,
                hidden_size=hidden_size
            ...
            model_weights_new['lstm.{}.weight_ih_l0_reverse'.format(l)] = W_ih_bw
            model_weights_new['lstm.{}.weight_hh_l0_reverse'.format(l)] = W_hh_bw
            model_weights_new['lstm.{}.bias_ih_l0_reverse'.format(l)] = bias_ih_bw
            bias_hh_bw[input_size: 2 * input_size] = torch.ones_like(bias_hh_bw[input_size: 2 * input_size])
            model_weights_new['lstm.{}.bias_hh_l0_reverse'.format(l)] = bias_hh_bw
    elif option.model_name == 'smDRUM' or option.model_name == 'mmDRUM':
        model_weights_ori = torch.load('{}/model3'.format(from_model_ckpt), map_location=torch.device('cpu'))
        model_weights_new = {'emb': model_weights_ori['query_embedding.weight'], 
                             'linear.weight': model_weights_ori['W_0'].T, 'linear.bias': model_weights_ori['b_0'].T.squeeze(-1)}
        for l in range(option.length):  
            model_weights_new['lstm.{}.weight_ih_l0'.format(l)] = model_weights_ori['cells.{}.weight_ih_l0'.format(l)]
            model_weights_new['lstm.{}.weight_hh_l0'.format(l)] = model_weights_ori['cells.{}.weight_hh_l0'.format(l)]
            model_weights_new['lstm.{}.bias_ih_l0'.format(l)] = model_weights_ori['cells.{}.bias_ih_l0'.format(l)]
            model_weights_new['lstm.{}.bias_hh_l0'.format(l)] = model_weights_ori['cells.{}.bias_hh_l0'.format(l)]
            
            model_weights_new['lstm.{}.weight_ih_l0_reverse'.format(l)] = model_weights_ori['cells.{}.weight_ih_l0_reverse'.format(l)]
            model_weights_new['lstm.{}.weight_hh_l0_reverse'.format(l)] = model_weights_ori['cells.{}.weight_hh_l0_reverse'.format(l)]
            model_weights_new['lstm.{}.bias_ih_l0_reverse'.format(l)] = model_weights_ori['cells.{}.bias_ih_l0_reverse'.format(l)]
            model_weights_new['lstm.{}.bias_hh_l0_reverse'.format(l)] = model_weights_ori['cells.{}.bias_hh_l0_reverse'.format(l)]

    for key in model_weights:
        print(key, model_weights[key].shape, model_weights_new[key].shape)
        assert model_weights_new[key].shape == model_weights[key].shape
    model.load_state_dict(model_weights_new)
    if option.use_gpu:
        model = model.cuda()
        torch.cuda.empty_cache()
    if option.use_soft: model.weight.cpu()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trained_path', default=None, type=str)
    ...
    parser.add_argument('--iteration_per_batch', default=10000, type=int)
