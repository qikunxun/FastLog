...
class Model(nn.Module):
    ...
    def forward(self, input_x, input_r, e2triple, triple2e, r2triple, is_training=False, input_y=None):
        ...
        for t in range(self.T):
            ...
            if t >= 1:
                x = states[-1]  # [b, L, E]
                s_i, s_h, s_t = sparse_matrix_multiply_L_sample(x, (e2triple[0], triple2e[1], r2triple[0], e2triple[2],
                                                                 w_probs), E, self.r_size, self.tau_1,
                                                                is_training, top_k=self.top_k,
                                                                topk_pruning=self.top_k_mask, is_max=True,
                                                                weight=self.weight, use_topk=self.use_topk)  # [b, L, E]
                s = s_i + s_h + s_t

            # print(t, s.detach().max())
