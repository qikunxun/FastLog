...
from utils import vectorized_operation_sp, max_sp, norm_sp, log_loss_common_sp, sparse_max
...
class Model(nn.Module):
    ...
    def forward(self, input_x, input_r, e2triple, triple2e, r2triple, is_training=False, input_y=None):
        ...
        for t in range(self.T):
            ...
            x = states[-1]
            s_i, s_h, s_t = vectorized_operation_sp(x,
                                            (e2triple[0], triple2e[1], r2triple[0], e2triple[2], w),
                                            E, self.r_size,  topk_pruning=self.c, is_max=True,
                                            weight=self.weight, use_topk=self.use_topk)  # [b, L, E]
            s = sparse_max(s_i, s_h, s_t)
